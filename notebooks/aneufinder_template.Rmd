...
long_aneu_scores edivisive_scores %>%
  filter(Chr %in% c(seq(1, 21), "X", "Y")) %>%
  ggplot(aes(x=SAMPLE_LABEL, y=Aneuploidy, fill=SAMPLE_LABEL)) +
  geom_boxplot(position="dodge") +
  facet_wrap("Chr", nrow=3, strip.position="bottom") +
  scale_fill_npg() +
  theme_bw() +
  theme(
    axis.ticks = element_blank(),
  ...
  labs(
    x="", title="Chromosome-wise aneuploidy scores (dnacopy) by condition"
  )
```